...
const Membership = () => {
    ...
    const [adEmail, setAdEmail] = useState(companyemail || "");
    ...
    const handleMembershipSubmit = async () => {
        const apiUrl = `${process.env.REACT_APP_BACKEND_DJANGO_URL}/company-membership/${companyid}/`;
    ...
    const handleAdvertisementSubmit = async () => {
        const apiUrl = `${process.env.REACT_APP_BACKEND_DJANGO_URL}/company-advertisement/${companyid}/`;
        ...
        try {
            const response = await axios.post(apiUrl, adBody, {
                headers: {
                    Authorization: `Bearer ${companytoken}`, // Include your token for auth if necessary
                },
            });
