...
const JobseekerForgotOtp = () => {
        ...
        if (csrfToken) {
            try {
                const response = await axios.post(
                    `${django_url}/jobseeker-verify-otp/`,
                    {
                        otp: enteredOtp,
                        // email: userEmail,
                    },
                ...
                if (response.data.message === "OTP verification successful") {
                    ...
                    navigate(`/jobseeker-forgot-password`);
                } else {
                    toast.error(
    ...
    const handleResendOTP = async () => {
        ...
        if (csrfToken) {
            try {
                const response = await axios.get(
                    `${django_url}/jobseeker-resend-otp/`,
                    {},
                    {
                        headers: {
                            "X-CSRFToken": csrfToken,
...
export default JobseekerForgotOtp;
